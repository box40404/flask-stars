document.addEventListener('DOMContentLoaded', async () => {
    ...
    function updatePrice() {
        const amount = Number(quantityInput.value) || 50;
        ...
        buyButton.textContent = `Купить ${amount} звёзд`;
    }
    ...
    if (window.Telegram && window.Telegram.WebApp) {
        ...
        if (savedPurchaseId) {
            ...
            quantityInput.value = localStorage.getItem('amount') || '50';
            ...
            fetch(`/api/purchase/${purchaseId}`)
                ...
                .then(data => {
                    if (data.error) {
                        ...
                        quantityInput.value = '';
                        usernameInput.value = usernameInput.value; // Сохраняем username, если он был
    ...
    quantityInput.addEventListener('input', () => {
        ...
        if (quantity) {
            starOptions.forEach(option => option.checked = false);
        }
        updatePrice();
    });
    ...
    buyButton.addEventListener('click', buyStars);
});